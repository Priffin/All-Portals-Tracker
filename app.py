...
class StrongholdTracker:
    def __init__(self):
        ...
        self.STRONGHOLD_RINGS = [
            (1280, 2816), (4352, 5888), (7424, 8960), 
            (10496, 12032), (13568, 15104), 
            (16640, 18176), (19712, 21248), (22784, 24320)
        ]
    ...
    def add_stronghold(self, coords, save=True):
        ...
        if len(self.first_eight_strongholds) == 8:
            ...
            for ring, (x, y) in self.first_eight_strongholds:
                self.socketio.emit('generate_point', (x, -y, f'p{self.stronghold_count}', '#8a0b11'))
            
            scaled_coords = [(t[1][0]*8, t[1][1]*8) for t in self.first_eight_strongholds]
            solver = Pathfinding()
    ...
    def next_stronghold(self):
        ...
        self.socketio.emit('generate_point', (self.target_coords[0], -self.target_coords[1], f'p{self.stronghold_count}'))
    ...
    def get_stronghold_ring(self, coords):
        ...
        return next((ring + 1 for ring, bounds in enumerate(self.STRONGHOLD_RINGS) 
                    if bounds[0] < dist < bounds[1]), 0)
    ...
    def get_record_path(self):
        ...
        try:
            with open(latest_world_dir, 'r') as f:
                data = json.load(f)
                return os.path.join(data.get('world_path'), 'speedrunigt', 'record.json')
    ...
    def count_strongholds(self, filepath):
        ...
        try:
            with open(filepath, 'r') as f:
                data = json.load(f)
                try:
                    self.stronghold_count = max(
                        int(t['name'][10:]) for t in data.get('timelines') 
                        if t['name'][:10] == 'portal_no_'
                    ) + self.skips
                except ValueError:
                    self.stronghold_count = 0

                self.update_number('stronghold_count', f'{self.stronghold_count}/{self.MAX_STRONGHOLDS}')
    ...
    def monitor_clipboard(self):
        ...
        while True: 
            ...
            try:
                ...
                coords = {
                    'x': float(components[6]) / 8 if dimension == 'overworld' else float(components[6]),
                    'y': float(components[7]),
                    'z': float(components[8]) / 8 if dimension == 'overworld' else float(components[8]),
                ...
                if self.target_coords:
                    ...
                    angle_delta = self.smallest_angle_difference(self.get_360_angle(coords['a']), stronghold_angle)
                    
                    angle_text = f'{"<--" if angle_delta > 0 else ""}{abs(angle_delta)}{"-->" if angle_delta <= 0 else ""}'
                    distance = self.distance_between_points((coords['x'], coords['z']), (sh_x, sh_z))
                    
                    self.update_number('angle', f'Angle: {round(stronghold_angle, 1)} ({angle_text})')
                ...
                for line in response.iter_lines():
                    if line:
                        try:
                            ...
                            self.ninbot_coords = (stronghold['chunkX'] * 2, stronghold['chunkZ'] * 2)
                        except Exception as e:
                            pass
